...
AIRBNB_PATH = os.path.abspath(os.path.join(
    os.path.dirname(__file__),
    "insideairbnb-listings.csv")
)

@st.cache
def get_data():
    return pd.read_csv(f"file://{AIRBNB_PATH}")
...
def get_availability(show_exp, neighborhood):
    return (df
        .query(f"""neighbourhood_group==@neighborhood{show_exp} and availability_365>0""")
        .availability_365.describe(percentiles=[.1, .25, .5, .75, .9, .99])
        .to_frame()
        .T
    )
...
st.map(df
    .query("price>=800")[["latitude", "longitude"]]
    .dropna(how="any")
)
st.markdown("_**Note:** More options are available with [`st.deck_gl_chart`](https://streamlit.io/docs/api.html#streamlit.deck_gl_chart)._")


# [write] ->  display data on a table (with sorting by default)
...
# [multiselect] ->  allow user to filter columns (still with sorting enable)
...
# [json] ->  display 2 selected hosts as JSON object
...
st.json({
    top_host_1.iloc[0].host_name: top_host_1[["name", "neighbourhood", "room_type", "minimum_nights", "price"]]
        .sample(2, random_state=4)
        .to_dict(orient="records"),
    top_host_2.iloc[0].host_name: top_host_2[["name", "neighbourhood", "room_type", "minimum_nights", "price"]]
        .sample(2, random_state=4)
        .to_dict(orient="records"),
})

# [sidebar.slider, plotly_chart] ->  diagram of price distribution
...
# [radio, checkbox] ->  another way to filter tables
...
# [pyplot] ->  diagram with matplotlib this time
...
# [sidebar.number_input] ->  another way to get user input
...
if minimum > maximum:
    ...
    st.write(df
        .query("@minimum<=number_of_reviews<=@maximum")
        .sort_values("number_of_reviews", ascending=False)
        .head(50)[["name", "number_of_reviews", "price", "neighbourhood", "host_name", "room_type"]]
    )
